...
**Ownership and scale carry equal weight with the vocabulary.** He has run programs: sole point of contact for access administration across a 40-person organization, sole IT resource on a 32-workstation enterprise migration, a safety program through external inspection with zero findings, 107 purchase actions with zero discrepancies. Wherever the access work appears, the reader should finish knowing he was accountable for it, not assisting with it. Show the scope and let it do the work.
...
- **Current role, canonical wording:** "IT systems technician and security manager liaison." Use it verbatim on every page, in JSON-LD `jobTitle`, and on the résumé. The NMCI Telecommunications Officer designation (17 Nov 2025) appears as detail in prose, never as the title. Collateral IT support duties since 2024, so the duration is two years, not three. Access administration for a 40-person organization in an RMF-governed federal environment. Headcount publishes as 40, settled Aug 31, 2026.
